...
class DemoSchedule extends React.Component{
    constructor(props) {
        super(props);
		this.state = {data:this.props.appointments,
		    currentDate:this.props.currentDate, name:"Anyone",
            teacher:0, family:0, student:0,typ:"student",
            edit:0, add_lesson:{},
            childKey:0, modal:false, show_item:0,
            days:this.props.days, zoom:0
        };
	...
	show_toggle(item=0){
        ...
        else{this.setState({modal:false, show_item:0})}
    }

	filter_student(id, typ="student", date=this.state.currentDate){
        console.log(date);
    ...
    save_lesson(resp){
        ...
        this.setState({edit:0, add_lesson:{}});
	...
	to_edit_lesson(){
		...
		fetch(manage_url, {method: "POST", body: formData}).then(response => response.json()).then((resp) => {
			console.log(resp);
			this.setState({edit:1, add_lesson: resp.lesson, modal:false, show_item:0})
		});
	}
    edit_lesson(lesson){
     ...
     cancel_lesson(lesson_id, single=0){
        ...
        if(confirm=== "yes"){
            ...
            fetch(manage_url, {method: "POST", body: formData}).then(response => response.json()).then((resp) => {
                    console.log(resp);
                    this.filter_student(this.state.teacher, "teacher",this.state.currentDate)
                this.setState({edit:0, add_lesson:{}, childKey:this.state.childKey+1});
            });
        }
    }
	render_timetable(){
			...
			<div>
				...
				<table className="table table-bordered table-striped">
					<thead>
						<tr>
							<th style={{width: "2%"}}></th>
							{this.days.map((item, index)=><th style={{width: "14%"}}>{item}, {this.state.days[index]}</th>)}
						</tr>
					</thead>
					<tbody>
						{this.hours.map((hour, i1)=>
						<tr>
							<td>{hour}:00</td>
							{this.days.map((day, i2)=>
							<td style={{padding:0}}>
	  ...
	  {show_item.duration_cost>=0?
        ...
        <div className="row">
            ...
            <select name="student" id="student" className="form-control"
                    onChange={(e)=>this.filter_student(e.target.value, "student")}
            ...
            <select name="family" id="family" className="form-control"
                    onChange={(e)=>this.filter_student(e.target.value, "family")}
            ...
            <select name="teacher" id="teacher" className="form-control"
                    onChange={(e)=>this.filter_student(e.target.value, "teacher")}
            ...
            <select name="zoom" id="zoom" className="form-control"
                    onChange={(e)=>this.filter_student(e.target.value, "zoom")}
            ...
            <div className="col-md-2 col-sm-6">
                ...
                <span>{this.state.name}</span> <span className="reset-btn" onClick={()=>this.filter_student(0, "reset")}>Reset</span>
            </div>
        </div>
                    ...
                    <div className="row">
                        <div className="col-md-3 col-sm-6 col-6 mb-3">
                            <button type="button" className="btn btn-lg btn-secondary mx-1"
                                onClick={()=>this.setState({edit:0, add_lesson:{}, childKey:this.state.childKey+1})}>Назад</button>
                            </div>
                            <div className="col-md-3 col-sm-6 col-6 mb-3">
                                <button type="button" className="btn btn-lg btn-warning mx-1"
                                onClick={()=>this.cancel_lesson(this.state.add_lesson.id, 1)}>Отменить урок</button>
                            </div>
                            <div className="col-md-3 col-sm-6 col-6 mb-3">
                                <button type="button" className="btn btn-lg btn-danger mx-1"
                                onClick={()=>this.cancel_lesson(this.state.add_lesson.id)}>Удалить</button>
                        </div>    
                    </div>
                </div>
